...
- 2 bugs found and fixed:
  1. Missing translation key `common.actions.edit` - Fixed by changing to `common.edit`
  2. Language name showing "Unknown" - Fixed TypeScript interface (language should not be array)
...
1. **Missing Translation Key**: Edit button showed `common.actions.edit` instead of "Edit"
   - **Root Cause**: Translation key `common.actions.edit` doesn't exist in messages files
   - **Fix**: Changed to `common.edit` in `components/descriptions/description-translation-form.tsx:198`
   - **File**: components/descriptions/description-translation-form.tsx
   - **Status**: ✅ Fixed

2. **Language Name Showing "Unknown"**: Breadcrumb showed "Unknown" instead of language name
   - **Root Cause**: TypeScript interface incorrectly defined `language` as array, but Supabase returns single object
   - **Fix**: Changed `language: LanguageWithTranslations[] | null` to `language: LanguageWithTranslations | null` in `app/actions/descriptions.ts:51` and updated access patterns
   - **Files**:
     - app/actions/descriptions.ts (interface and 2 access patterns)
   - **Status**: ✅ Fixed
